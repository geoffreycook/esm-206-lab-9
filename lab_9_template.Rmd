...
# a. Saturated model: 
## This is DANGEROUS since including the variable PricePerSqFt since the overall Price variable is DERIVED from the parameter PricePerSqFt.

## Concerns about overly specified model with colinearity
...
## Changing what is the dummy variable (what you are basing your measurements on)
homes_sub$Status <- factor(homes_sub$Status)

# not changing any of the data or observations, we are just telling R to look at the data as a factor instead of a character - now we can use 'forcats'
...
# c. Check levels:
# use the fct_relevel() function in forcats package
...
full_aic <- AIC(homes_lm1) # 10699
final_aic <- AIC(homes_lm3) # 11147

# Answer: which would you pick? 

# You would choose lm1
```
